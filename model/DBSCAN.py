...
class VisionDBSCAN(DBSCAN):
    def __init__(self, eps, min_samples):
        ...
        self.yolo = YOLOIterator("../weights/yolov8_best.pt")
    ...
    def vision_fit(self, X, frame):
        ...
        if id is not None:
            self._distance(X)
            R = in_box(xyxy, X)
            for k, iPoint in enumerate(R):
                for i in range(len(iPoint)):
                    if self.visit[i] == 1:
                        continue
                    if R[k, i] == True:
                        self.label[i] = id[k]
            ...
            for i in range(n_samples):
                if self.visit[i] == 1 and self.label[i] != 0:
                    cluster = self.label[i]
                    neighbors = self._region_query(i)
                    if len(neighbors) >= self.min_samples:
                        self._expand_cluster(neighbors, cluster)
        return self.label.astype(int)
